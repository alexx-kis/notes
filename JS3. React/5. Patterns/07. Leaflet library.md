...
- Мы создадим два вида маркеров:
  ...
  - и currentCustomIcon — для отображения выбранного пользователем маркера В нашем примере маркеры отличаются только параметром `iconUrl`, он принимает URL-путь к изображению
      ...
      points.forEach((point) => {
        leaflet
          .marker(
            ...
            {
              icon: point.title === selectedPoint.title ? currentCustomIcon : defaultCustomIcon,
            },
          )
          .addTo(map);
      });
